...
def graph_broken(df, title, outfile, ylim1, ylim2, ymax):
    ...
    rect2 = ax1.bar(ind + width, df.baseline, width, color=duck_color, label="DuckDB")

    rect1 = ax2.bar(ind, df.arachne, width, color=arachne_color, label="Arachne")
    rect2 = ax2.bar(ind + width, df.baseline, width, color=duck_color, label="DuckDB")
...
def graph_df(df, labels, colors, keys, title, outfile):
    ...
    rect2 = ax.bar(ind + width, df.baseline, width, color=duck_color, label="Baseline")
...
if __name__ == '__main__':
    ...
    for k in raw:
        ...
        cost = vals['total'] + b['bq_cost'] + b['duck_cost'] + b['duck_c_cost']
        num = vals['len'] + 3
        if num == 4:
        ...
        percent = 100*(comp - cost) / comp
        if cost < comp:
            print(f"{k}: percent difference: {percent} %")
    ...
    dic = {"keys":keys, "baseline": baseline_costs, "arachne":arachne_costs,
            "hybrid": found_hybrid, "percents": percents}
    df = pd.DataFrame(dic)
    print(len(df))

    labels = ["BigQuery", "DuckDB", "Arachne"]
    title = "Arachne, BigQuery, and DuckDB Cost per query ($)"
    ...
    labels = ["BigQuery", "DuckDB", "Arachne"]
    title = "Arachne Profiling Cost ($) versus Equivalent DuckDB Runs (Profile more expensive)"
    colors = [bq_color, rs_color, arachne_color]
    outfile = "gcp_profile_expensive.png"
    graph_df(df_hybrid, labels, colors, keys, title, outfile)
    # graph_broken(df_hybrid, title, outfile, 3, 15, 25)

    title = "Arachne Profiling Cost ($) versus Equivalent DuckDB Runs (Profile cheaper)"
    outfile = "gcp_profile_cheaper.png"
    graph_df(df_static, labels, colors, keys, title, outfile)
    # graph_broken(df_static, title, outfile, 1, 1, 5)
