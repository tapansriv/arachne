...
def graph_broken(df, labels, colors, keys, title, outfile):
    ...
    rect1 = ax1.bar(ind, df.duck, width, color=arachne_color, label="Arachne wrt DuckDB")
    rect2 = ax1.bar(ind + width, df.bq, width, color=duck_color, label="Arachne Savings wrt BigQuery")
...
def graph_df(df, labels, colors, keys, title, outfile):
    ...
    rect1 = ax.bar(ind, df.duck, width, color=duck_rel_color, label="Arachne wrt DuckDB")
    ...
    ax.set_ylabel("% Difference in Cost")
...
if __name__ == '__main__':
    ...
    for k in raw:
        ...
        duck_percent = 100 * ((duck_cost - arachne_cost) / duck_cost)
        bq_savings.append(bq_percent)
        duck_savings.append(duck_percent)

    arachne_color = "#ecae17"
    bq_color = "#455984"
    rs_color = "#a12721"

    dic = {"keys":keys, "bq": bq_savings, "duck": duck_savings}
    df = pd.DataFrame(dic)
    print(len(df))

    df_static = df[df.duck == 0]
    df_hybrid = df[df.duck != 0]
    print(len(df_hybrid))
    print(len(df_static))

    labels = ["BigQuery", "DuckDB", "Arachne"]
    title = "Arachne Percent Savings versus DuckDB, BigQuery"
    colors = [bq_color, rs_color, arachne_color]
    outfile = "gcp_rel_cluster1.png"
    graph_df(df_static, labels, colors, keys, title, outfile)

    outfile = "gcp_rel_cluster2.png"
