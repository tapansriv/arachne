...
FROM
  (SELECT DISTINCT c_last_name,
   ...
   WHERE store_sales.ss_sold_date_sk = date_dim.d_date_sk
     ...
     DISTINCT SELECT c_last_name,
                     c_first_name,
                     d_date
     FROM `arachne-multicloud.tpcds.catalog_sales` AS catalog_sales,
          `arachne-multicloud.tpcds.date_dim` AS date_dim,
          `arachne-multicloud.tpcds.customer` AS customer WHERE catalog_sales.cs_sold_date_sk = date_dim.d_date_sk
     ...
     FROM `arachne-multicloud.tpcds.web_sales` AS web_sales,
          `arachne-multicloud.tpcds.date_dim` AS date_dim,
          `arachne-multicloud.tpcds.customer` AS customer WHERE web_sales.ws_sold_date_sk = date_dim.d_date_sk
