...
FROM tpcds.sf1000.catalog_returns AS catalog_returns) AS "t0" ON "t"."cs_order_number" = "t0"."cr_order_number" AND "t"."cs_item_sk" = "t0"."cr_item_sk") AS "t1"
INNER JOIN (SELECT "w_warehouse_sk", "w_state"
FROM tpcds.sf1000.warehouse AS warehouse) AS "t2" ON "t1"."cs_warehouse_sk" = "t2"."w_warehouse_sk") AS "t3"
...
ORDER BY "t7"."w_state" IS NULL, "t7"."w_state", "t7"."i_item_id" IS NULL, "t7"."i_item_id"
LIMIT 100
