...
FROM tpcds.sf1000.call_center AS call_center) AS "t"
INNER JOIN (SELECT "cr_returned_date_sk", "cr_returning_customer_sk", "cr_call_center_sk", "cr_net_loss"
FROM tpcds.sf1000.catalog_returns AS catalog_returns) AS "t0" ON "t"."cc_call_center_sk" = "t0"."cr_call_center_sk") AS "t1"
...
FROM tpcds.sf1000.customer AS customer) AS "t6" ON "t5"."cr_returning_customer_sk" = "t6"."c_customer_sk") AS "t7"
