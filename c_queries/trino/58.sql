...
FROM tpcds.sf1000.item AS item) AS "t0" ON "t"."ss_item_sk" = "t0"."i_item_sk") AS "t1"
INNER JOIN (SELECT "date_dim"."d_date_sk"
FROM tpcds.sf1000.date_dim AS date_dim
INNER JOIN (SELECT "date_dim0"."d_date"
FROM tpcds.sf1000.date_dim AS "date_dim0"
...
FROM tpcds.sf1000.catalog_sales AS catalog_sales) AS "t15"
INNER JOIN (SELECT "i_item_sk", "i_item_id"
FROM tpcds.sf1000.item AS item) AS "t16" ON "t15"."cs_item_sk" = "t16"."i_item_sk") AS "t17"
INNER JOIN (SELECT "date_dim2"."d_date_sk"
FROM tpcds.sf1000.date_dim AS "date_dim2"
INNER JOIN (SELECT "date_dim3"."d_date"
FROM tpcds.sf1000.date_dim AS "date_dim3"
LEFT JOIN (SELECT SINGLE_VALUE("d_week_seq") AS "$f0"
FROM tpcds.sf1000.date_dim AS date_dim
...
FROM tpcds.sf1000.web_sales AS web_sales) AS "t32"
INNER JOIN (SELECT "i_item_sk", "i_item_id"
FROM tpcds.sf1000.item AS item) AS "t33" ON "t32"."ws_item_sk" = "t33"."i_item_sk") AS "t34"
INNER JOIN (SELECT "date_dim5"."d_date_sk"
FROM tpcds.sf1000.date_dim AS "date_dim5"
INNER JOIN (SELECT "date_dim6"."d_date"
FROM tpcds.sf1000.date_dim AS "date_dim6"
LEFT JOIN (SELECT SINGLE_VALUE("d_week_seq") AS "$f0"
FROM tpcds.sf1000.date_dim AS date_dim
