...
FROM tpcds.sf1000.customer_demographics AS customer_demographics) AS "t4" ON "t3"."c_current_cdemo_sk" = "t4"."cd_demo_sk") AS "t5"
INNER JOIN (SELECT "hd_demo_sk", "hd_income_band_sk"
FROM tpcds.sf1000.household_demographics AS household_demographics) AS "t6" ON "t5"."c_current_hdemo_sk" = "t6"."hd_demo_sk") AS "t7"
INNER JOIN (SELECT "ib_income_band_sk"
FROM tpcds.sf1000.income_band AS income_band
...
FROM tpcds.sf1000.store_returns AS store_returns) AS "t12" ON "t11"."cd_demo_sk" = "t12"."sr_cdemo_sk"
ORDER BY "t11"."c_customer_id"
LIMIT 100
