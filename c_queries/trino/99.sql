...
FROM tpcds.sf1000.warehouse AS warehouse) AS "t1" ON "t"."cs_warehouse_sk" = "t1"."w_warehouse_sk") AS "t2"
INNER JOIN (SELECT "sm_ship_mode_sk", "sm_type"
FROM tpcds.sf1000.ship_mode AS ship_mode) AS "t3" ON "t2"."cs_ship_mode_sk" = "t3"."sm_ship_mode_sk") AS "t4"
INNER JOIN (SELECT "cc_call_center_sk", "cc_name"
FROM tpcds.sf1000.call_center AS call_center) AS "t5" ON "t4"."cs_call_center_sk" = "t5"."cc_call_center_sk") AS "t7"
...
ORDER BY "t7"."w_substr", "t7"."sm_type", LOWER("t7"."cc_name")
LIMIT 100
