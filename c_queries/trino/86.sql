...
FROM tpcds.sf1000.item AS item) AS "t4" ON "t3"."ws_item_sk" = "t4"."i_item_sk"
...
FROM (SELECT "ws_sold_date_sk", "ws_item_sk", "ws_net_paid"
FROM tpcds.sf1000.web_sales AS web_sales) AS "t8"
INNER JOIN (SELECT "d_date_sk"
FROM tpcds.sf1000.date_dim AS date_dim
WHERE "d_month_seq" >= 1200 AND "d_month_seq" <= 1200 + 11) AS "t11" ON "t8"."ws_sold_date_sk" = "t11"."d_date_sk") AS "t12"
INNER JOIN (SELECT "i_item_sk", "i_class", "i_category"
FROM tpcds.sf1000.item AS item) AS "t13" ON "t12"."ws_item_sk" = "t13"."i_item_sk"
...
FROM (SELECT "ws_sold_date_sk", "ws_item_sk", "ws_net_paid"
FROM tpcds.sf1000.web_sales AS web_sales) AS "t20"
INNER JOIN (SELECT "d_date_sk"
FROM tpcds.sf1000.date_dim AS date_dim
WHERE "d_month_seq" >= 1200 AND "d_month_seq" <= 1200 + 11) AS "t23" ON "t20"."ws_sold_date_sk" = "t23"."d_date_sk") AS "t24"
INNER JOIN (SELECT "i_item_sk", "i_class", "i_category"
FROM tpcds.sf1000.item AS item) AS "t25" ON "t24"."ws_item_sk" = "t25"."i_item_sk"
