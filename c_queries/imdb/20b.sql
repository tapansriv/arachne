SELECT MIN("t24"."title") AS "complete_downey_ironman_movie"
FROM (SELECT "t17"."movie_id", "t17"."movie_id0", "t17"."id5", "t17"."movie_id1"
...
(SELECT "id"
FROM char_name
WHERE ("name" LIKE '%Tony%Stark%' OR "name" LIKE '%Iron%Man%') AND "name" NOT LIKE '%Sherlock%') AS "t8"
...
(SELECT "id"
FROM keyword
WHERE "keyword" IN ('based-on-comic', 'fight', 'marvel-comics', 'second-part', 'sequel', 'superhero', 'tv-special', 'violence')) AS "t13"
CROSS JOIN 
(SELECT "id"
FROM kind_type
...
INNER JOIN (SELECT "id"
FROM name
WHERE "name" LIKE '%Downey%Robert%') AS "t20" ON "t17"."person_id" = "t20"."id") AS "t21"
INNER JOIN (SELECT "id", "title", "kind_id"
FROM title
WHERE "production_year" > 2000) AS "t24" ON "t21"."id5" = "t24"."kind_id" AND "t21"."movie_id1" = "t24"."id" AND "t21"."movie_id0" = "t24"."id" AND "t21"."movie_id" = "t24"."id"
;
