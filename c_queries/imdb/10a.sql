SELECT MIN("t12"."name") AS "uncredited_voiced_character", MIN("t15"."title") AS "russian_movie"
...
FROM cast_info
WHERE "note" LIKE '%(voice)%' AND "note" LIKE '%(uncredited)%') AS "t2" ON "t"."id" = "t2"."person_role_id") AS "t3"
CROSS JOIN 
(SELECT "id"
FROM company_name
...
FROM title
WHERE "production_year" > 2005) AS "t15" ON "t12"."movie_id0" = "t15"."id" AND "t12"."movie_id" = "t15"."id"
;
