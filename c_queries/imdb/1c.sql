...
WHERE "note" LIKE '%(co-production)%' AND "note" NOT LIKE '%(as Metro-Goldwyn-Mayer Pictures)%') AS "t4" ON "t0"."id" = "t4"."company_type_id") AS "t5"
...
FROM title
WHERE "production_year" > 2010) AS "t9" ON "t7"."movie_id" = "t9"."id" AND "t7"."movie_id0" = "t9"."id"
;
