...
UNION ALL
SELECT "t50"."i_manufact_id", SUM("t44"."ws_ext_sales_price") AS "total_sales"
...
FROM web_sales) AS "t36"
INNER JOIN (SELECT "d_date_sk"
FROM date_dim
WHERE "d_year" = 1998 AND "d_moy" = 5) AS "t39" ON "t36"."ws_sold_date_sk" = "t39"."d_date_sk") AS "t40"
INNER JOIN (SELECT "ca_address_sk"
FROM customer_address
WHERE "ca_gmt_offset" = -5) AS "t43" ON "t40"."ws_bill_addr_sk" = "t43"."ca_address_sk") AS "t44"
INNER JOIN (SELECT "item3"."i_item_sk", "item3"."i_manufact_id"
FROM item AS "item3"
INNER JOIN (SELECT "i_manufact_id"
FROM item
WHERE "i_category" = 'Electronics'
GROUP BY "i_manufact_id") AS "t48" ON "item3"."i_manufact_id" = "t48"."i_manufact_id") AS "t50" ON "t44"."ws_item_sk" = "t50"."i_item_sk"
GROUP BY "t50"."i_manufact_id") AS "t54"
GROUP BY "i_manufact_id"
ORDER BY SUM("total_sales") IS NULL, SUM("total_sales")
LIMIT 100
;
