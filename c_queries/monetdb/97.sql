SELECT CAST(COUNT(*) FILTER (WHERE "t4"."customer_sk" IS NOT NULL AND "t10"."customer_sk" IS NULL) AS SIGNED) AS "store_only", CAST(COUNT(*) FILTER (WHERE "t4"."customer_sk" IS NULL AND "t10"."customer_sk" IS NOT NULL) AS SIGNED) AS "catalog_only", CAST(COUNT(*) FILTER (WHERE "t4"."customer_sk" IS NOT NULL AND "t10"."customer_sk" IS NOT NULL) AS SIGNED) AS "store_and_catalog"
