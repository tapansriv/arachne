SELECT "t59"."customer_id0" AS "customer_id", "t59"."customer_first_name0" AS "customer_first_name", "t59"."customer_last_name0" AS "customer_last_name"
...
WHERE "sale_type" = 'w' AND "year_" = 2001 AND "year_total" > 0) AS "t58" ON "t38"."customer_id" = "t58"."customer_id") AS "t59"
INNER JOIN (SELECT "customer_id", "year_total"
FROM (SELECT "t62"."c_customer_id" AS "customer_id", "t62"."c_first_name" AS "customer_first_name", "t62"."c_last_name" AS "customer_last_name", "t64"."d_year" AS "year_", SUM("t62"."ss_net_paid") AS "year_total", 's' AS "sale_type"
FROM (SELECT "t60"."c_customer_id", "t60"."c_first_name", "t60"."c_last_name", "t61"."ss_sold_date_sk", "t61"."ss_net_paid"
FROM (SELECT "c_customer_sk", "c_customer_id", "c_first_name", "c_last_name"
FROM customer) AS "t60"
INNER JOIN (SELECT "ss_sold_date_sk", "ss_customer_sk", "ss_net_paid"
FROM store_sales) AS "t61" ON "t60"."c_customer_sk" = "t61"."ss_customer_sk") AS "t62"
INNER JOIN (SELECT "d_date_sk", "d_year"
FROM date_dim
WHERE "d_year" = 2001 OR "d_year" = 2001 + 1) AS "t64" ON "t62"."ss_sold_date_sk" = "t64"."d_date_sk"
GROUP BY "t62"."c_customer_id", "t62"."c_first_name", "t62"."c_last_name", "t64"."d_year"
UNION ALL
SELECT "t70"."c_customer_id" AS "customer_id", "t70"."c_first_name" AS "customer_first_name", "t70"."c_last_name" AS "customer_last_name", "t72"."d_year" AS "year_", SUM("t70"."ws_net_paid") AS "year_total", 'w' AS "sale_type"
FROM (SELECT "t68"."c_customer_id", "t68"."c_first_name", "t68"."c_last_name", "t69"."ws_sold_date_sk", "t69"."ws_net_paid"
FROM (SELECT "c_customer_sk", "c_customer_id", "c_first_name", "c_last_name"
FROM customer) AS "t68"
INNER JOIN (SELECT "ws_sold_date_sk", "ws_bill_customer_sk", "ws_net_paid"
FROM web_sales) AS "t69" ON "t68"."c_customer_sk" = "t69"."ws_bill_customer_sk") AS "t70"
INNER JOIN (SELECT "d_date_sk", "d_year"
FROM date_dim
WHERE "d_year" = 2001 OR "d_year" = 2001 + 1) AS "t72" ON "t70"."ws_sold_date_sk" = "t72"."d_date_sk"
GROUP BY "t70"."c_customer_id", "t70"."c_first_name", "t70"."c_last_name", "t72"."d_year") AS "t76"
WHERE "sale_type" = 'w' AND "year_" = 2001 + 1) AS "t79" ON "t59"."customer_id" = "t79"."customer_id" AND CASE WHEN "t59".">" THEN CAST("t79"."year_total" / "t59"."year_total1" AS DECIMAL(19, 0)) ELSE NULL END > "t59"."CASE"
ORDER BY "t59"."customer_id0"
LIMIT 100
;
