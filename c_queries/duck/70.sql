...
FROM '/mnt/disks/tpcds/parquet/store.parquet' AS "store1"
INNER JOIN (SELECT "s_state"
FROM (SELECT "t24"."s_state", RANK() OVER (PARTITION BY "t24"."s_state" ORDER BY "t24"."$f1" DESC) AS "ranking"
...
'/mnt/disks/tpcds/parquet/store.parquet' AS "store2",
'/mnt/disks/tpcds/parquet/date_dim.parquet' AS "date_dim2"
...
FROM (SELECT "ss_sold_date_sk", "ss_store_sk", "ss_net_profit"
FROM '/mnt/disks/tpcds/parquet/store_sales.parquet' AS store_sales) AS "t36"
INNER JOIN (SELECT "d_date_sk"
FROM '/mnt/disks/tpcds/parquet/date_dim.parquet' AS date_dim
...
FROM '/mnt/disks/tpcds/parquet/store.parquet' AS "store3"
INNER JOIN (SELECT "s_state"
FROM (SELECT "t44"."s_state", RANK() OVER (PARTITION BY "t44"."s_state" ORDER BY "t44"."$f1" DESC) AS "ranking"
...
'/mnt/disks/tpcds/parquet/store.parquet' AS "store4",
