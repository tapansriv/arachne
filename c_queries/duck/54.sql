...
INNER JOIN '/mnt/disks/tpcds/parquet/customer.parquet' AS customer ON "t"."customer_sk" = "customer"."c_customer_sk"
UNION ALL
SELECT *
FROM (SELECT "ws_sold_date_sk" AS "sold_date_sk", "ws_bill_customer_sk" AS "customer_sk", "ws_item_sk" AS "item_sk"
FROM '/mnt/disks/tpcds/parquet/web_sales.parquet' AS web_sales) AS "t2"
INNER JOIN (SELECT *
FROM '/mnt/disks/tpcds/parquet/item.parquet' AS item
...
INNER JOIN '/mnt/disks/tpcds/parquet/customer.parquet' AS "customer0" ON "t2"."customer_sk" = "customer0"."c_customer_sk") AS "t5"
GROUP BY "c_customer_sk", "c_current_addr_sk") AS "t7"
INNER JOIN (SELECT "ss_sold_date_sk", "ss_customer_sk", "ss_ext_sales_price"
FROM '/mnt/disks/tpcds/parquet/store_sales.parquet' AS store_sales) AS "t8" ON "t7"."c_customer_sk" = "t8"."ss_customer_sk") AS "t9"
INNER JOIN (SELECT "ca_address_sk", "ca_county", "ca_state"
FROM '/mnt/disks/tpcds/parquet/customer_address.parquet' AS customer_address) AS "t10" ON "t9"."c_current_addr_sk" = "t10"."ca_address_sk") AS "t11"
INNER JOIN (SELECT "s_county", "s_state"
FROM '/mnt/disks/tpcds/parquet/store.parquet' AS store) AS "t12" ON "t11"."ca_county" = "t12"."s_county" AND "t11"."ca_state" = "t12"."s_state") AS "t13"
INNER JOIN (SELECT "date_dim1"."d_date_sk"
FROM '/mnt/disks/tpcds/parquet/date_dim.parquet' AS "date_dim1"
...
ORDER BY "t29"."SEGMENT" NULLS FIRST, COUNT(*) NULLS FIRST, "t29"."SEGMENT" * 50
