...
FROM '/mnt/disks/tpcds/parquet/store.parquet' AS store) AS "t",
(SELECT "i_item_sk", "i_item_desc", "i_current_price", "i_wholesale_cost", "i_brand"
FROM '/mnt/disks/tpcds/parquet/item.parquet' AS item) AS "t0",
...
FROM '/mnt/disks/tpcds/parquet/store_sales.parquet' AS store_sales) AS "t1"
...
FROM (SELECT "ss_sold_date_sk", "ss_item_sk", "ss_store_sk", "ss_sales_price"
FROM '/mnt/disks/tpcds/parquet/store_sales.parquet' AS store_sales) AS "t10"
INNER JOIN (SELECT "d_date_sk"
FROM '/mnt/disks/tpcds/parquet/date_dim.parquet' AS date_dim
...
ORDER BY "t"."s_store_name" NULLS FIRST, "t0"."i_item_desc" NULLS FIRST
