...
INNER JOIN (SELECT *
FROM '/mnt/disks/tpcds/parquet/inventory.parquet' AS inventory) AS "t0" ON "t"."cs_item_sk" = "t0"."inv_item_sk" AND "t"."cs_quantity" > "t0"."inv_quantity_on_hand") AS "t1"
INNER JOIN (SELECT "w_warehouse_sk", "w_warehouse_name"
FROM '/mnt/disks/tpcds/parquet/warehouse.parquet' AS warehouse) AS "t2" ON "t1"."inv_warehouse_sk" = "t2"."w_warehouse_sk") AS "t3"
INNER JOIN (SELECT "i_item_sk", "i_item_desc"
FROM '/mnt/disks/tpcds/parquet/item.parquet' AS item) AS "t4" ON "t3"."cs_item_sk" = "t4"."i_item_sk") AS "t5"
...
FROM '/mnt/disks/tpcds/parquet/date_dim.parquet' AS date_dim) AS "t18" ON "t17"."inv_date_sk" = "t18"."d_date_sk" AND "t17"."d_week_seq" = "t18"."d_week_seq") AS "t19"
INNER JOIN (SELECT "d_date_sk", "d_date"
FROM '/mnt/disks/tpcds/parquet/date_dim.parquet' AS date_dim) AS "t20" ON "t19"."cs_ship_date_sk" = "t20"."d_date_sk" AND "t19"."+" < "t20"."d_date") AS "t21"
LEFT JOIN (SELECT "p_promo_sk"
FROM '/mnt/disks/tpcds/parquet/promotion.parquet' AS promotion) AS "t22" ON "t21"."cs_promo_sk" = "t22"."p_promo_sk") AS "t23"
LEFT JOIN (SELECT "cr_item_sk", "cr_order_number"
FROM '/mnt/disks/tpcds/parquet/catalog_returns.parquet' AS catalog_returns) AS "t24" ON "t23"."cs_item_sk" = "t24"."cr_item_sk" AND "t23"."cs_order_number" = "t24"."cr_order_number"
