SELECT *
FROM (SELECT CASE WHEN "t5"."item_sk" IS NOT NULL THEN "t5"."item_sk" ELSE "t12"."item_sk" END AS "item_sk", CASE WHEN "t5"."d_date" IS NOT NULL THEN "t5"."d_date" ELSE "t12"."d_date" END AS "d_date", "t5"."cume_sales" AS "web_sales", "t12"."cume_sales" AS "store_sales", MAX("t5"."cume_sales") OVER (PARTITION BY CASE WHEN "t5"."item_sk" IS NOT NULL THEN "t5"."item_sk" ELSE "t12"."item_sk" END ORDER BY CASE WHEN "t5"."d_date" IS NOT NULL THEN "t5"."d_date" ELSE "t12"."d_date" END ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS "web_cumulative", MAX("t12"."cume_sales") OVER (PARTITION BY CASE WHEN "t5"."item_sk" IS NOT NULL THEN "t5"."item_sk" ELSE "t12"."item_sk" END ORDER BY CASE WHEN "t5"."d_date" IS NOT NULL THEN "t5"."d_date" ELSE "t12"."d_date" END ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS "store_cumulative"
FROM (SELECT "t4"."item_sk", "t4"."d_date", SUM("t4"."$f2") OVER (PARTITION BY "t4"."item_sk" ORDER BY "t4"."d_date" ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS "cume_sales"
...
FULL JOIN (SELECT "t11"."item_sk", "t11"."d_date", SUM("t11"."$f2") OVER (PARTITION BY "t11"."item_sk" ORDER BY "t11"."d_date" ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS "cume_sales"
...
FROM '/mnt/disks/tpcds/parquet/store_sales.parquet' AS store_sales) AS "t6"
INNER JOIN (SELECT "d_date_sk", "d_date"
FROM '/mnt/disks/tpcds/parquet/date_dim.parquet' AS date_dim
