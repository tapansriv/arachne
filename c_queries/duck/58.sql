...
FROM '/mnt/disks/tpcds/parquet/item.parquet' AS item) AS "t0" ON "t"."ss_item_sk" = "t0"."i_item_sk") AS "t1"
...
LEFT JOIN (SELECT SINGLE_VALUE("d_week_seq") AS "$f0"
FROM '/mnt/disks/tpcds/parquet/date_dim.parquet' AS date_dim
...
FROM '/mnt/disks/tpcds/parquet/catalog_sales.parquet' AS catalog_sales) AS "t15"
INNER JOIN (SELECT "i_item_sk", "i_item_id"
FROM '/mnt/disks/tpcds/parquet/item.parquet' AS item) AS "t16" ON "t15"."cs_item_sk" = "t16"."i_item_sk") AS "t17"
...
INNER JOIN (SELECT "i_item_sk", "i_item_id"
FROM '/mnt/disks/tpcds/parquet/item.parquet' AS item) AS "t33" ON "t32"."ws_item_sk" = "t33"."i_item_sk") AS "t34"
INNER JOIN (SELECT "date_dim5"."d_date_sk"
FROM '/mnt/disks/tpcds/parquet/date_dim.parquet' AS "date_dim5"
INNER JOIN (SELECT "date_dim6"."d_date"
FROM '/mnt/disks/tpcds/parquet/date_dim.parquet' AS "date_dim6"
LEFT JOIN (SELECT SINGLE_VALUE("d_week_seq") AS "$f0"
FROM '/mnt/disks/tpcds/parquet/date_dim.parquet' AS date_dim
...
ORDER BY "t31"."item_id" NULLS FIRST, "t31"."ss_item_rev" NULLS FIRST
