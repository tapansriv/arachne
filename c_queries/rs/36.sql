...
INNER JOIN (SELECT "i_item_sk", "i_class", "i_category"
FROM item) AS "t17" ON "t16"."ss_item_sk" = "t17"."i_item_sk") AS "t18"
INNER JOIN (SELECT "s_store_sk"
FROM store
WHERE "s_state" = 'TN') AS "t21" ON "t18"."ss_store_sk" = "t21"."s_store_sk"
GROUP BY "t18"."i_category", "t18"."i_class") AS "t24"
GROUP BY "t24"."i_category")
UNION
SELECT SUM(SUM("t34"."ss_net_profit")) * 1.0000 / SUM(SUM("t34"."ss_ext_sales_price")) AS "gross_margin", NULL AS "i_category", NULL AS "i_class", 1 AS "t_category", 1 AS "t_class", 2 AS "lochierarchy"
