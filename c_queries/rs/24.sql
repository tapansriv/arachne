...
FROM store_returns) AS "t0" ON "t"."ss_ticket_number" = "t0"."sr_ticket_number" AND "t"."ss_item_sk" = "t0"."sr_item_sk") AS "t1"
...
FROM item) AS "t6" ON "t5"."ss_item_sk" = "t6"."i_item_sk") AS "t7"
...
FROM customer_address) AS "t10" ON "t9"."c_current_addr_sk" = "t10"."ca_address_sk" AND "t9"."c_birth_country" <> "t10"."UPPER" AND "t9"."s_zip" = "t10"."ca_zip"
...
LEFT JOIN (SELECT SINGLE_VALUE("t23"."EXPR$0") AS "$f0"
FROM (SELECT 0.05 * AVG(SUM("store_sales0"."ss_net_paid")) AS "EXPR$0"
...
GROUP BY "customer0"."c_last_name", "customer0"."c_first_name", "store0"."s_store_name", "customer_address0"."ca_state", "store0"."s_state", "item0"."i_color", "item0"."i_current_price", "item0"."i_manager_id", "item0"."i_units", "item0"."i_size") AS "t23") AS "t24" ON TRUE
WHERE "t16"."paid" > "t24"."$f0"
ORDER BY "t16"."c_last_name", "t16"."c_first_name", "t16"."s_store_name"
