...
LEFT JOIN (SELECT SINGLE_VALUE(AVG("ss_net_profit")) AS "$f0"
FROM (VALUES (NULL, NULL, NULL)) AS "t" ("ss_addr_sk", "ss_store_sk", "ss_net_profit")
WHERE 1 = 0
GROUP BY "ss_store_sk") AS "t7" ON TRUE
...
FROM (VALUES (NULL, NULL, NULL)) AS "t" ("ss_addr_sk", "ss_store_sk", "ss_net_profit")
WHERE 1 = 0
GROUP BY "ss_store_sk") AS "t20" ON TRUE
...
FROM item) AS "t27" ON "t26"."item_sk" = "t27"."i_item_sk") AS "t28"
INNER JOIN (SELECT "i_item_sk", "i_product_name"
FROM item) AS "t29" ON "t28"."item_sk0" = "t29"."i_item_sk"
ORDER BY "t28"."rnk"
LIMIT 100
