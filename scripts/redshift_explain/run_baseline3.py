...
def run_original(key):
    ...
    conn = redshift_connector.connect(
        host='redshift-cluster-3.cm5xisyashnz.us-east-2.redshift.amazonaws.com',
        port=5439,
        database='dev',
    ...
    print(orig_qry)
    print(f"starting original query {key}; redshift-cluster-3")
    start = time.time()
    try:
...
if __name__ == '__main__':
    ...
    if runtime != -1:
        home = os.path.expanduser("~")
        x = None
        with open(f"{home}/arachneDB/data/rs_c_baseline_1_2tb.json") as fp:
            x = json.load(fp)

        x[key] = runtime
        with open(f"{home}/arachneDB/data/rs_c_baseline_1_2tb.json", "w") as fp:
