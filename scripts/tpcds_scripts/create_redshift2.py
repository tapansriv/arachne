...
print(len(tables))
print("redshift-cluster-2")

conn = redshift_connector.connect(
    host='redshift-cluster-2.cm5xisyashnz.us-east-2.redshift.amazonaws.com',
    port=5439,
    database='dev',
...
try:
    for tbl in tables:
        ...
        load = f"COPY {tbl} FROM 's3://arachne-multicloud/tpcds/{tbl}.parquet' iam_role " + auth;
        if tbl in weird_tables:
            load = f"COPY {tbl} FROM 's3://arachne-multicloud/{tbl}.parquet' iam_role " + auth;
...
print(times)
# with open("rs4_load_times_1tb.json", "w") as fp:
