...
class GCPObjectStore(ObjectStore):
    ...
    def gcp_region(self):
        ...
        try:
            ...
            raise
        if bucket is None:
            raise exceptions.MissingBucketException(f"GCP bucket {self.bucket_name} does not exist")
        else:
    ...
    def create_bucket(self, gcp_region, premium_tier=True):
        assert premium_tier, "Standard tier GCP buckets are not supported"
    ...
    def list_objects(self, prefix="") -> Iterator[GCPObject]:
        blobs = self._gcs_client.list_blobs(self.bucket_name, prefix=prefix)
        for blob in blobs:
            yield GCPObject("gcs", self.bucket_name, blob.name, blob.size, blob.updated, mime_type=getattr(blob, "content_type", None))
    ...
    def complete_multipart_upload(self, dst_object_name, upload_id):
        ...
        try:
            ...
            raise Exception("Failed to complete multipart upload") from e
