...
def find_best_subset(bm, query_bit_vecs, n, egress_cost, use_duck=False,
    ...
    init_cost, init_runtime = compute_plan_stats(bm, curr_query_set,
            curr_table_set, keys, final_queries, final_tables)
    
    plan_cache[init_cost] = {'runtime': init_runtime,
    ...
    if SLA is None:
        ...
        # valid_plans includes the default execution method (migrating no tables # or queries)
        # So if that is empty, that means the runtime SLA is too strict even for
        # the default method, which is really unsupported behavior; i.e. setting
        # a runtime bound of 10 seconds for a workload that takes 2 hours can't
        # be supported.
        valid_plans = [cost for cost in plan_cache if plan_cache[cost]['runtime'] < SLA]
        if len(valid_plans) == 0:
            raise ValueError("Runtime SLA provided is too strict even for default execution plan that migrates no data or queries. No valid execution plans exist then")
        best_cost = min(valid_plans)
...
if __name__ == '__main__':
    ...
    if args.workload_outfile != '':
        bm.get_workload_runtime_and_size(len(table_indices), args.workload_outfile,
                                         exclude_type=exclude_type,
                                         exclude=exclude,
                                         query_bit_vecs=query_bit_vecs)
    else: 
        curr_table_set, curr_query_set = find_best_subset(bm,
                query_bit_vecs, len(table_indices), egress, use_duck=args.duck,
