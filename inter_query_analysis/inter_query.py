...
def find_best_subset(baselineManager, query_bit_vecs, n, egress_cost,
    ...
    plan_type = ""
    if multi_cloud_plan and len(curr_table_set) > 0 and len(curr_table_set) < num_tables_start:
        # truly multi cloud
        plan_type = "multi"
    elif len(curr_table_set) == 0:
        # gcp only
        plan_type = "gcp"
    elif len(curr_table_set) == num_tables_start:
        # aws only; we remove no tables
        plan_type = "aws"
    elif not multi_cloud_plan and len(curr_table_set) > 0:
        # aws only; we removed tables; no table had any deps => multi_cloud_plan false
        plan_type = "aws"
    else:
        plan_type = "wtf"
    ...
    total_runtime += 1800 # TODO MOVEMENT RUNTIME? 1/2 an hour (SHIT ESTIMATE)
    total_runtime += rs_total_load_time
    final_cost = (covered_costs + uncovered_costs + mvmt_cost + rs_total_load_cost + storage_cost) # all dollars
