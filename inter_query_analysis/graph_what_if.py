...
for bq in bqs:
    ...
    if args.no_calcite:
        data_file = f"{dir_}/outputs_NOCALCITE_{fname}_{src}_0.12_{rs_cost}_{bq_key}{duck_txt}.json"
    # print(data_file)
...
if args.no_calcite:
    outfile = f"graphs/sf{SF}_cs{CS}_NOCALCITE_{fname}_{src}_what_if_bq.pdf"
...
for e2 in egresses:
    e = round(e2, 3)
    data_file = f"{dir_}/outputs_{fname}_{src}_{e}_{rs_cost}_6.25{duck_txt}.json"
    if args.no_calcite:
        data_file = f"{dir_}/outputs_NOCALCITE_{fname}_{src}_{e}_{rs_cost}_6.25{duck_txt}.json"
    print(data_file)
...
ax.plot(egresses, multi_e, label=f"Multi: GA1", marker='*', ms=markersize,
        c=arachne_color, zorder=3)
ax.plot(egresses, all_src, label=src_label, marker='o', ms=markersize,
        c=bq_color, zorder=2)
ax.plot(egresses, all_dest, label=dest_label, marker='D', ms=markersize, c=duck_color,
        zorder=1)
...
if args.legend:
    plt.legend()
# plt.show()
outfile = f"graphs/sf{SF}_cs{CS}_{fname}_{src}_what_if_egress.pdf"
if args.no_calcite:
    outfile = f"graphs/sf{SF}_cs{CS}_NOCALCITE_{fname}_{src}_what_if_egress.pdf"
# figure = plt.gcf()
