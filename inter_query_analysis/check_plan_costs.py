...
def process_subset(bm, table_set, moved_queries, stayed_queries, query_bit_vecs, n, use_duck=False, exclude_type=None, exclude=None):
...
if __name__ == '__main__':
    ...
    parser.add_argument("check_path", help="Path where estimates are")
    ...
    print( 
          f"{args.check_path}/outputs_NOCALCITE_noduck_{args.start_loc.upper()}_{egress}_{cluster_rs_cost}_{args.bq_cost}.json"
    )
    initial_profile = json.load(
            open(
                f"{args.check_path}/outputs_NOCALCITE_noduck_{args.start_loc.upper()}_{egress}_{cluster_rs_cost}_{args.bq_cost}.json"
                )
            )
    ...
    table_names = initial_profile[key]["exec_details"]["moved_tables"]
    moved_queries = {q: 0 for q in initial_profile[key]["exec_details"]["moved_queries"]}
    stayed_queries = {q: 0 for q in initial_profile[key]["exec_details"]["stayed_queries"]}
    ...
    process_subset(bm, table_set, moved_queries, stayed_queries, query_bit_vecs,
                   len(table_indices), use_duck=args.duck,
