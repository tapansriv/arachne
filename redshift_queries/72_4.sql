CREATE TABLE rs_table_72_0 AS SELECT "t19"."cs_item_sk", "t19"."cs_promo_sk", "t19"."cs_order_number", "t19"."w_warehouse_name", "t19"."i_item_desc", "t19"."d_week_seq"
...
FROM date_dim) AS "t20" ON "t19"."cs_ship_date_sk" = "t20"."d_date_sk" AND "t19"."+" < "t20"."d_date"
;
SELECT "t0"."i_item_desc", "t0"."w_warehouse_name", "t0"."d_week_seq", SUM("t0"."CASE") AS "no_promo", SUM("t0"."CASE6") AS "promo", COUNT(*) AS "total_cnt"
FROM (SELECT "rs_table_72_0"."cs_item_sk", "rs_table_72_0"."cs_order_number", "rs_table_72_0"."w_warehouse_name", "rs_table_72_0"."i_item_desc", "rs_table_72_0"."d_week_seq", CASE WHEN "t"."p_promo_sk" IS NULL THEN 1 ELSE 0 END AS "CASE", CASE WHEN "t"."p_promo_sk" IS NOT NULL THEN 1 ELSE 0 END AS "CASE6"
FROM "rs_table_72_0"
LEFT JOIN (SELECT "p_promo_sk"
FROM promotion) AS "t" ON "rs_table_72_0"."cs_promo_sk" = "t"."p_promo_sk") AS "t0"
LEFT JOIN (SELECT "cr_item_sk", "cr_order_number"
FROM catalog_returns) AS "t1" ON "t0"."cs_item_sk" = "t1"."cr_item_sk" AND "t0"."cs_order_number" = "t1"."cr_order_number"
GROUP BY "t0"."i_item_desc", "t0"."w_warehouse_name", "t0"."d_week_seq"
ORDER BY COUNT(*) DESC, "t0"."i_item_desc" NULLS FIRST, "t0"."w_warehouse_name" NULLS FIRST, "t0"."d_week_seq" NULLS FIRST
LIMIT 100
;
