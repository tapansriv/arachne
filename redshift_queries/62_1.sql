CREATE TABLE rs_table_62_0 AS SELECT "t7"."w_substr", "t7"."sm_type", "t7"."web_name", "t7"."CASE" AS "$f3", "t7"."CASE5" AS "$f4", "t7"."CASE6" AS "$f5", "t7"."CASE7" AS "$f6", "t7"."CASE8" AS "$f7"
...
;
SELECT "w_substr", "sm_type", "web_name", SUM("$f3") AS "30 days", SUM("$f4") AS "31-60 days", SUM("$f5") AS "61-90 days", SUM("$f6") AS "91-120 days", SUM("$f7") AS ">120 days"
FROM "rs_table_62_0"
GROUP BY "w_substr", "sm_type", "web_name"
ORDER BY "w_substr" NULLS FIRST, "sm_type" NULLS FIRST, "web_name" NULLS FIRST
LIMIT 100
;
