CREATE TABLE rs_table_73_0 AS SELECT "t7"."ss_ticket_number", "t7"."ss_customer_sk"
...
HAVING COUNT(*) >= 1 AND COUNT(*) <= 5) AS "t0"
INNER JOIN (SELECT "c_customer_sk", "c_salutation", "c_first_name", "c_last_name", "c_preferred_cust_flag"
FROM customer) AS "t1" ON "t0"."ss_customer_sk" = "t1"."c_customer_sk"
ORDER BY "t0"."cnt" DESC NULLS LAST, "t1"."c_last_name"
;
