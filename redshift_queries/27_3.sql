CREATE TABLE rs_table_27_0 AS SELECT "t12"."i_item_id", "t11"."ss_quantity" AS "agg1", "t11"."ss_list_price" AS "agg2", "t11"."ss_coupon_amt" AS "agg3", "t11"."ss_sales_price" AS "agg4"
...
GROUP BY "i_item_id")
UNION ALL
SELECT NULL AS "i_item_id", NULL AS "s_state", 1 AS "g_state", AVG("t30"."ss_quantity") AS "agg1", AVG("t30"."ss_list_price") AS "agg2", AVG("t30"."ss_coupon_amt") AS "agg3", AVG("t30"."ss_sales_price") AS "agg4"
FROM (SELECT "t26"."ss_item_sk", "t26"."ss_quantity", "t26"."ss_list_price", "t26"."ss_sales_price", "t26"."ss_coupon_amt"
FROM (SELECT "t22"."ss_item_sk", "t22"."ss_store_sk", "t22"."ss_quantity", "t22"."ss_list_price", "t22"."ss_sales_price", "t22"."ss_coupon_amt"
FROM (SELECT "t18"."ss_sold_date_sk", "t18"."ss_item_sk", "t18"."ss_store_sk", "t18"."ss_quantity", "t18"."ss_list_price", "t18"."ss_sales_price", "t18"."ss_coupon_amt"
FROM (SELECT "ss_sold_date_sk", "ss_item_sk", "ss_cdemo_sk", "ss_store_sk", "ss_quantity", "ss_list_price", "ss_sales_price", "ss_coupon_amt"
FROM store_sales) AS "t18"
INNER JOIN (SELECT "cd_demo_sk"
FROM customer_demographics
WHERE "cd_gender" = 'M' AND "cd_marital_status" = 'S' AND "cd_education_status" = 'College') AS "t21" ON "t18"."ss_cdemo_sk" = "t21"."cd_demo_sk") AS "t22"
INNER JOIN (SELECT "d_date_sk"
FROM date_dim
WHERE "d_year" = 2002) AS "t25" ON "t22"."ss_sold_date_sk" = "t25"."d_date_sk") AS "t26"
INNER JOIN (SELECT "s_store_sk"
FROM store
WHERE "s_state" = 'TN') AS "t29" ON "t26"."ss_store_sk" = "t29"."s_store_sk") AS "t30"
INNER JOIN (SELECT "i_item_sk"
FROM item) AS "t31" ON "t30"."ss_item_sk" = "t31"."i_item_sk") AS "t35"
