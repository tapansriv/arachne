 ...
 */
with recursive
srcs(f) as (select 28587302322191),
dsts(t) as (select 26388279066632),
myForums(id) as (
    select f_forumid as id from 'forum.parquet' f where to_timestamp(f.f_creationdate/1000) between '2012-12-08' and '2012-12-12'
),
mm as (
    ...
    from 'knows.parquet' k, 
         'message.parquet' msg, 
         'message.parquet' reply
    where true
...
path(src, dst, w) as (
    select k.k_person1id, k.k_person2id, 10::double precision / (coalesce(w, 0) + 10)
    from 'knows.parquet' k left join mm on least(k.k_person1id, k.k_person2id) = mm.src and greatest(k.k_person1id, k.k_person2id) = mm.dst
),
