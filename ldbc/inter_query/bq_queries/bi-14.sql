...
,  case1 AS (
    SELECT DISTINCT
      ...
      FROM person1_list p1
         , person2_list p2
         , ldbc.message m -- ldbc.message by p2
         , ldbc.message r -- reply by p1
     WHERE 1=1
        -- join
...
,  case2 AS (
    SELECT DISTINCT
      ...
      FROM person1_list p1
         , person2_list p2
         , ldbc.message m -- ldbc.message by p1
         , ldbc.message r -- reply by p2
     WHERE 1=1
        -- join
       AND m.m_messageid = r.m_c_replyof
...
,  score_ranks AS (
    SELECT s.person1id
      ...
      FROM ldbc.place co -- country
           INNER JOIN ldbc.place ci ON (co.pl_placeid = ci.pl_containerplaceid) -- city
...
SELECT s.person1id AS person1id
     , s.person2id AS person2id
     , s.cityName AS city1name
     , s.score
  FROM score_ranks s
 WHERE 1=1
    -- filter
