...
SELECT au.p_personid AS personid
     , au.p_firstname AS personfirstName
     , au.p_lastname AS personlastName
     , au.p_creationdate
     -- a single ldbc.person might be member of more than 1 of the top100 forums, so their posts should be DISTINCT counted
     , count(DISTINCT p.m_messageid) AS postCount
  FROM top100_popular_forums t
       INNER JOIN ldbc.forum_person fp ON (t.forumid = fp.fp_forumid)
       -- author of the post
       INNER JOIN ldbc.person au ON (fp.fp_personid = au.p_personid)
       LEFT JOIN ldbc.message p ON (1=1
                        AND au.p_personid = p.m_creatorid
                        AND p.m_c_replyof IS NULL
                           )
 WHERE p.m_ps_forumid IN (SELECT forumid from top100_popular_forums)
