select p_personid, p_firstname, p_lastname, l.l_creationdate, m_messageid,
  ...
  --EXTRACT(EPOCH FROM (l.l_creationdate - m_creationdate)) / 60 as lag,
    (case when exists (select 1 from ldbc.knows where k_person1id = 21990232556256 and k_person2id = p_personid) then 0 else 1 end) as isnew
from
  (select l_personid, max(l_creationdate) as l_creationdate
   from ldbc.likes, ldbc.message
   where
   ...
   order by 2 desc
   limit 20
  ) tmp, ldbc.message, ldbc.person, ldbc.likes as l
where
